...
# Calculate x^2 and x*log(x) for comparison
x_squared = [x**2 for x in input_sizes]
x_log_x = [x * math.log(x) for x in input_sizes]
...
# Add perfect plots for x^2 and x*log(x)
#plt.plot(input_sizes, x_squared, label='x^2', linestyle='--')
plt.plot(input_sizes, x_log_x, label='x*log(x)', linestyle='--')

plt.xlabel('Input Size (n)')
plt.ylabel('Number of Steps')
plt.title('Number of Steps vs Input Size with x^2 and x*log(x) Comparison')
plt.legend()
plt.grid(True)
plt.show()
