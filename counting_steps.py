...
def partition(arr, low, high):
    ...
    for j in range(low, high):
        steps += 1  # Count the comparison arr[j] < pivot
        if arr[j] < pivot:
            i += 1
            arr[i], arr[j] = arr[j], arr[i]
            steps += 1  # Count the swap
    arr[i + 1], arr[high] = arr[high], arr[i + 1]
    steps += 1  # Count the final swap
    return i + 1, steps
...
def quick_sort(arr, low, high):
    ...
    if low < high:
        ...
        steps += partition_steps  # Add the steps from partition
        steps += quick_sort(arr, low, pi - 1)  # Count steps in the left recursive call
        steps += quick_sort(arr, pi + 1, high)  # Count steps in the right recursive call
    return steps
